package net.mcreator.minecraftdrugs.block.entity;
...
public class DistilleryTileEntity extends RandomizableContainerBlockEntity implements GeoBlockEntity, WorldlyContainer {
	private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
	private NonNullList<ItemStack> stacks = NonNullList.<ItemStack>withSize(6, ItemStack.EMPTY);
	private final SidedInvWrapper handler = new SidedInvWrapper(this, null);
	public int blockstateNew = this.getBlockState().getValue(DistilleryBlock.BLOCKSTATE);
	private int blockstateOld = this.getBlockState().getValue(DistilleryBlock.BLOCKSTATE);

	public DistilleryTileEntity(BlockPos pos, BlockState state) {
		super(MinecraftDrugsModBlockEntities.DISTILLERY.get(), pos, state);
	}

	private PlayState predicate(AnimationState event) {
		blockstateNew = this.getBlockState().getValue(DistilleryBlock.BLOCKSTATE);
		if (blockstateOld != blockstateNew) {
			event.getController().forceAnimationReset();
			blockstateOld = blockstateNew;
			return PlayState.STOP;
		}
		String animationprocedure = ("" + this.getBlockState().getValue(DistilleryBlock.ANIMATION));
	...
	public void registerControllers(AnimatableManager.ControllerRegistrar data) {
		data.add(new AnimationController<DistilleryTileEntity>(this, "controller", 0, this::predicate));
		data.add(new AnimationController<DistilleryTileEntity>(this, "procedurecontroller", 0, this::procedurePredicate));
	}
	...
	public AbstractContainerMenu createMenu(int id, Inventory inventory) {
		return new DistilleryGUIMenu(id, inventory, new FriendlyByteBuf(Unpooled.buffer()).writeBlockPos(this.worldPosition));
	}
