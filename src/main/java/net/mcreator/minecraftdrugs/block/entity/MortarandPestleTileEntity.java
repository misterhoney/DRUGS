...
public class MortarandPestleTileEntity extends RandomizableContainerBlockEntity implements GeoBlockEntity, WorldlyContainer {
	private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
	private NonNullList<ItemStack> stacks = NonNullList.<ItemStack>withSize(3, ItemStack.EMPTY);
	private final SidedInvWrapper handler = new SidedInvWrapper(this, null);

	public MortarandPestleTileEntity(BlockPos pos, BlockState state) {
		super(MinecraftDrugsModBlockEntities.MORTARAND_PESTLE.get(), pos, state);
	}

	private PlayState predicate(AnimationState event) {
		String animationprocedure = ("" + this.getBlockState().getValue(MortarandPestleBlock.ANIMATION));
	...
	public void registerControllers(AnimatableManager.ControllerRegistrar data) {
		data.add(new AnimationController<MortarandPestleTileEntity>(this, "controller", 0, this::predicate));
		data.add(new AnimationController<MortarandPestleTileEntity>(this, "procedurecontroller", 0, this::procedurePredicate));
	}
	...
	public Component getDefaultName() {
		return Component.literal("mortarand_pestle");
	}
	...
	public AbstractContainerMenu createMenu(int id, Inventory inventory) {
		return new MotarMenu(id, inventory, new FriendlyByteBuf(Unpooled.buffer()).writeBlockPos(this.worldPosition));
	}
